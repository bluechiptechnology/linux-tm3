...
#define XRM117XX_DLD_REG		(0x02) /* Divisor Latch Fractional */
...
struct xrm117x_port {
	...
	struct clk				*uart24Mhzclk;
	struct clk				*wifi32Khzclk;
	...
	u8			iostate_reg_cache;
	u8			iointena_reg_cache;
	u8			ier_reg_cache;
...
static u8 xrm117x_port_read(struct uart_port *port, u8 reg)
{
	...
	if(status < 0)
	{
	    printk("Failed to xrm117x_port_read error code %ld\n",status);
	}
	return result;
...
static void xrm117x_port_write(struct uart_port *port, u8 reg, u8 val)
{
	...
	if(status < 0)
	{
	    printk("Failed to xrm117x_port_write Err_code %ld\n",status);
	}
	mutex_unlock(&s->mutex_bus_access);
...
void xrm117x_raw_write(struct uart_port *port,const void *reg,unsigned char *buf,int len)
{
	...
	//printk("xrm117x_raw_write len:%d\n", len);
	...
	if(status < 0)
	{
	    printk("Failed to xrm117x_raw_write Err_code %ld %d\n",status, len);
	}
	mutex_unlock(&s->mutex_bus_access);
...
void xrm117x_raw_read(struct uart_port *port,unsigned char *buf,int len)
{
	...
	if(status < 0)
	{
	    printk("Failed to xrm117x_raw_read Err_code %ld\n",status);
...
static int xrm117x_set_baud(struct uart_port *port, int baud)
{
	u8 lcr,tmp;
	u8 prescaler = 0;
	unsigned long clk = port->uartclk, div = clk / 8 / baud;
 	if (div > 0xffff) {
	...
	xrm117x_port_write(port, XRM117XX_DLD_REG, 0x10); //X8 mode
...
static int xrm117x_dump_register(struct uart_port *port)
{
	...
	xrm117x_port_write(port, XRM117XX_LCR_REG, 0x80);
	printk("******Dump register at LCR=0x80\n");
	for (i = 0; i<16; i++)
	{
		reg = xrm117x_port_read(port, i);
		printk("Reg[0x%02x] = 0x%02x\n", i, reg);
...
static void xrm117x_handle_tx(struct uart_port *port)
{
	...
	unsigned int txlen, rxlen, to_send, i;
	unsigned char thr_reg;
	int batchsend = 0;

 	if (unlikely(port->x_char))
	{
		//printk("xrm117x_handle_tx: x_char\r\n");
	...
	//while(to_send)
	{
		...
		batchsend = (to_send > txlen) ? txlen : to_send;

		if (batchsend == 0)
		{
			rxlen = xrm117x_port_read(port, XRM117XX_RXLVL_REG);
			printk("xrm117x_handle_tx tx buffer full: %d %d\r\n", batchsend, rxlen);
		...
		*/
		...
		/* Add data to send */
		port->icount.tx += batchsend;

		/* Convert to linear buffer */
		for (i = 0; i < batchsend; ++i)
		{
		...
		xrm117x_raw_write(port, &thr_reg, s->buf, batchsend);
...
static void xrm117x_port_irq(struct xrm117x_port *s, int portno)
{
	...
	do {
		...
		//int iIndexCounter = 0;
		//struct gpio_chip * gpiochip = &s->gpio;
		...
		{	
			if (rxlen == 0 && to_send == 0)
			{
				break;
			}
			else if (rxlen != 0)
			{
				iir = XRM117XX_IIR_RDI_SRC;
			}
			else if (to_send != 0)
			{
				iir = XRM117XX_IIR_THRI_SRC;
			}
		}
		...
		case XRM117XX_IIR_CTSRTS_SRC:
			...
			uart_handle_cts_change(port, !!(msr & XRM117XX_MSR_CTS_BIT));
		...
		#endif	
		/*
			case XRM117XX_IIR_INPIN_SRC:

			msr = xrm117x_port_read(port, XRM117XX_IOSTATE_REG);
			for (iIndexCounter = 0; iIndexCounter < gpiochip->ngpio; iIndexCounter++)
			{
				
				if (s->iointena_reg_cache & BIT(iIndexCounter))
				{
					child_irq = irq_find_mapping(s->gpio.irqdomain, iIndexCounter);
					handle_nested_irq(child_irq);

					
					if ((s->irq_rise & BIT(iIndexCounter)) && (msr & BIT(iIndexCounter)) && !(s->iostate_reg_cache & BIT(iIndexCounter)))
					{
						child_irq = irq_find_mapping(s->gpio.irqdomain, iIndexCounter);
						handle_nested_irq(child_irq);
						//dev_err(port->dev, "Handle rising irq: %d\n", iIndexCounter);
					}
					else if ((s->irq_fall & BIT(iIndexCounter)) && !(msr & BIT(iIndexCounter)) && (s->iostate_reg_cache & BIT(iIndexCounter)))
					{
						dev_err(port->dev, "Handle falling irq: %d %x\n", iIndexCounter, msr);
						child_irq = irq_find_mapping(s->gpio.irqdomain, iIndexCounter);
						//handle_nested_irq(child_irq);
					}
...
static void xrm117x_wq_proc(struct work_struct *ws)
{
	...
	returncode = mutex_trylock(&s->mutex);
	if (!returncode)
	{
		//If the mutex is already locked th ISR will handle data transmission
		return;
	}

	if (s->suspended)
	{
		//If we are suspended do not transmit data
		mutex_unlock(&s->mutex);
		return;
	}
				  ...
				  struct ktermios *old)
{
	...
	{
		//printk("Enable auto RTS CTS\r\n");
		flow |= XRM117XX_EFR_AUTORTS_BIT | XRM117XX_EFR_AUTOCTS_BIT;
	...
	baud = uart_get_baud_rate(port, termios, old,
				  port->uartclk / 8 / 4 / 0xffff,
				  port->uartclk / 8);

	printk("xrm117x_set_baud: %d %d\r\n", port->uartclk,  baud);
...
static int xrm117x_startup(struct uart_port *port)
{
	...
	/* Reset FIFOs*/
	val = XRM117XX_FCR_RXRESET_BIT | XRM117XX_FCR_TXRESET_BIT;// | 0xF0;
	xrm117x_port_write(port, XRM117XX_FCR_REG, val);
	udelay(5);
	xrm117x_port_write(port, XRM117XX_FCR_REG,
		XRM117XX_FCR_FIFO_BIT);// | 0xF0);
	...
	xrm117x_port_write(port, XRM117XX_TCR_REG,
			     XRM117XX_TCR_RX_RESUME(12) |
			     XRM117XX_TCR_RX_HALT(16));
...
static int __maybe_unused xrm117x_suspend(struct device *dev)
{
	struct xrm117x_port *s = dev_get_drvdata(dev);

	//printk("xrm117x_suspend+\r\n");

	uart_suspend_port(&s->uart, &s->p[0].port);
	s->suspended = 1;
	mutex_lock(&s->mutex);
	mutex_unlock(&s->mutex);
	...
	//s->ier_reg_cache = xrm117x_port_read(&s->p[0].port, XRM117XX_IER_REG);
	//xrm117x_port_write(&s->p[0].port, XRM117XX_IER_REG, XRM117XX_IER_SLEEP_BIT);
	//mutex_unlock(&s->mutex);
...
static int __maybe_unused xrm117x_resume(struct device *dev)
{
	struct xrm117x_port *s = dev_get_drvdata(dev);
	...
	//mutex_lock(&s->mutex);
	//xrm117x_port_write(&s->p[0].port, XRM117XX_IER_REG, s->ier_reg_cache);
	s->suspended = 0;
	uart_resume_port(&s->uart, &s->p[0].port);
...
static SIMPLE_DEV_PM_OPS(xrm117x_pm_ops, xrm117x_suspend, xrm117x_resume);
...
static int xrm117x_gpio_direction_input(struct gpio_chip *chip, unsigned offset)
{
	struct xrm117x_port *s = container_of(chip, struct xrm117x_port, gpio);
	struct uart_port *port = &s->p[0].port;
...
//GPIO Irqs did not work as expected so comment out for now. Occasional system lockups were observed.
/*
static void xrm117x_irq_mask(struct irq_data *data)
{
	struct gpio_chip * gpiochip = irq_data_get_irq_chip_data(data);
	struct xrm117x_port *s = gpiochip_get_data(gpiochip);
	//struct uart_port *port = &s->p[0].port;
	

	unsigned int pos = data->hwirq;

	s->iointena_reg_cache &= ~BIT(pos);
}

static void xrm117x_irq_unmask(struct irq_data *data)
{
	struct gpio_chip * gpiochip = irq_data_get_irq_chip_data(data);
	struct xrm117x_port *s = gpiochip_get_data(gpiochip);
	unsigned int pos = data->hwirq;
...
static int xrm117x_irq_set_type(struct irq_data *data, unsigned int type)
{
...
static void xrm117x_irq_bus_lock(struct irq_data *data)
{
	struct gpio_chip * gpiochip = irq_data_get_irq_chip_data(data);
	struct xrm117x_port *s = gpiochip_get_data(gpiochip);

	//printk("xrm117x_irq_bus_lock\r\n");

	mutex_lock(&s->irq_lock);
}

static void xrm117x_irq_bus_unlock(struct irq_data *data)
{
	struct gpio_chip * gpiochip = irq_data_get_irq_chip_data(data);
	struct xrm117x_port *s = gpiochip_get_data(gpiochip);
	struct uart_port *port = &s->p[0].port;
	...
	xrm117x_port_write(port, XRM117XX_IOINTENA_REG, s->iointena_reg_cache);

	mutex_unlock(&s->mutex);

	//printk("xrm117x_irq_bus_unlock\r\n");
	mutex_unlock(&s->irq_lock);
}

static struct irq_chip xrm117x_irq_chip = {
	.name = "gpio-xrm117x",
	.irq_mask = xrm117x_irq_mask,
	.irq_unmask = xrm117x_irq_unmask,
	.irq_set_type = xrm117x_irq_set_type,
	.irq_bus_lock = xrm117x_irq_bus_lock,
	.irq_bus_sync_unlock = xrm117x_irq_bus_unlock,
};


static int xrm117x_irq_setup(struct xrm117x_port * s)
{
	struct gpio_chip *chip = &s->gpio;
	int err;
			   ...
			   int irq, unsigned long flags)
{
    ...
    #if 1
	s->uart24Mhzclk = devm_clk_get(dev, "clock24mhz");
	if (IS_ERR(s->uart24Mhzclk))
	{
		return PTR_ERR(s->uart24Mhzclk);
	} 
	else 
	{
		freq = clk_get_rate(s->uart24Mhzclk);
		printk("xrm117x_probe: Got uart24Mhzclk freq: %ld\r\n", freq);

		//DPR fixme. Should only enable while the uart is opened
		clk_prepare_enable(s->uart24Mhzclk);
	}

	s->wifi32Khzclk = devm_clk_get(dev, "clock32khz");
	if (IS_ERR(s->wifi32Khzclk))
	{
		return PTR_ERR(s->wifi32Khzclk);
	}
	else
	{
		freq = clk_get_rate(s->wifi32Khzclk);
		printk("xrm117x_probe: Got wifi32Khzclk freq: %ld\r\n", freq);

		//DPR fixme. Should only enable while the uart is opened
		clk_prepare_enable(s->wifi32Khzclk);
	}
	#else
    freq = 24000000;//Fixed the clk freq 
	#endif
	...
	if (devtype->nr_gpio) {
		...
		s->gpio.can_sleep	 = 0;
		s->gpio.parent = dev;
		//s->gpio.of_node = dev->of_node;
		s->gpio.of_gpio_n_cells = 2;
		s->gpio.irq_not_threaded = 0;
	...
	for (i = 0; i < devtype->nr_uart; ++i) {
		...
		xrm117x_port_write(&s->p[i].port, XRM117XX_IOCONTROL_REG, 0x01);//configure GPIO 4-7 as GPIO with latching on
		//xrm117x_port_write(&s->p[i].port, XRM117XX_IOINTENA_REG, 0x40);
	...
	ret = devm_request_threaded_irq(dev, irq, NULL, xrm117x_ist, IRQF_ONESHOT | IRQF_TRIGGER_FALLING | flags, dev_name(dev), s);

	/*
	s->irq_controller = of_property_read_bool(s->gpio.of_node, "interrupt-controller");
	if (s->irq && s->irq_controller) {
		status = xrm117x_irq_setup(s);
		if (status) {
			goto fail;
		}
	}
...
{
	const struct xrm117x_devtype *devtype = &xrm117x_devtype;
