...
{
	int retval;
	unsigned long base, oldbase,  len;
	struct uart_port serial_port;
	...
	oldbase = pci_resource_start(dev, FL_BASE1);

	base = oldbase & 0x0000FFFF;
	base |= 0x05410000;

	serial_port.mapbase = base;
	serial_port.membase = ioremap(base,len);

	DEBUG("membase=0x%x\n oldmapbase=0x%x mapbase=0x%x\n",(unsigned int)serial_port.membase,(unsigned int)serial_port.mapbase, (unsigned int) oldbase);
	DEBUG("value at address 3FC is %x\n",readl(serial_port.membase + 0x3FC));
	writel(0,serial_port.membase + 0x3FC);
	DEBUG("value at address 3FC after configuring is %x\n",readl(serial_port.membase + 0x3FC));
	base = pci_resource_start(dev,FL_BASE0);
...
#else
	pci_enable_msi(dev);
	if ((retval = request_irq(dev->irq, serial99xx_nocascade_interrupt,0,"saturn",&serial99xx_ports[retval])))
	{
		goto disable;
	}
#endif
