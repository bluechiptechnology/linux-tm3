...
static int enable_pcap_multitouch = 0;
...
static DEVICE_ATTR(irq_enable, 0644, ft5x06_irq_status, ft5x06_irq_enable);
...
void ft5x06_xy_worker(struct work_struct *work)
{
    ...
    retval = i2c_smbus_read_i2c_block_data(ts->client, FT5x06_WMREG_DEVICE_MODE, 0x1F, (u8 *)&tch_data);
    if (retval < 0)
    {
    ...
    {
        x1 = GET_COORDINATE(tch_data.x1_l,tch_data.x1_h);
        x2 = GET_COORDINATE(tch_data.x2_l,tch_data.x2_h);
		x3 = GET_COORDINATE(tch_data.x3_l,tch_data.x3_h);
        x4 = GET_COORDINATE(tch_data.x4_l,tch_data.x4_h);
		x5 = GET_COORDINATE(tch_data.x5_l,tch_data.x5_h);
        y1 = GET_COORDINATE(tch_data.y1_l,tch_data.y1_h);
        y2 = GET_COORDINATE(tch_data.y2_l,tch_data.y2_h);
		y3 = GET_COORDINATE(tch_data.y3_l,tch_data.y3_h);
        y4 = GET_COORDINATE(tch_data.y4_l,tch_data.y4_h);
		y5 = GET_COORDINATE(tch_data.y5_l,tch_data.y5_h);


        id_1 = (tch_data.y1_h>>4);
        id_2 = (tch_data.y2_h>>4);
		id_3 = (tch_data.y3_h>>4);
		id_4 = (tch_data.y4_h>>4);
		id_5 = (tch_data.y5_h>>4);
    ...
    switch (cur_tch)
    {
		case 5:
        {
            ...
            if (tilt)
            {
                FLIP_XY(g_xy_data.x5, g_xy_data.y5);
            }

            if (rev_x)
            {
                g_xy_data.x5 = INVERT_X(g_xy_data.x5, ts->platform_data->maxx);
            }

            if (rev_y)
            {
                g_xy_data.y5 = INVERT_Y(g_xy_data.y5, ts->platform_data->maxy-1);
            }
            ...
            if (ts->platform_data->use_trk_id)
            {
                cur_mt_pos[FT_MT_TCH5_IDX][FT_XPOS] = g_xy_data.x5;
                cur_mt_pos[FT_MT_TCH5_IDX][FT_YPOS] = g_xy_data.y5;
            }
            else
            {
                cur_mt_pos[id][FT_XPOS] = g_xy_data.x5;
                cur_mt_pos[id][FT_YPOS] = g_xy_data.y5;
                cur_mt_z[id] = FT_MAXZ;
            }

            cur_mt_tch[FT_MT_TCH5_IDX] = id;
            cur_trk[id] = FT_TCH;
            /* do not break */
        }
		case 4:
        {
            ...
            if (rev_y)
            {
                g_xy_data.y4 = INVERT_Y(g_xy_data.y4, ts->platform_data->maxy-1);
            }
            ...
            if (ts->platform_data->use_trk_id)
            {
                cur_mt_pos[FT_MT_TCH4_IDX][FT_XPOS] = g_xy_data.x4;
                cur_mt_pos[FT_MT_TCH4_IDX][FT_YPOS] = g_xy_data.y4;
            }
            else
            {
                cur_mt_pos[id][FT_XPOS] = g_xy_data.x4;
                cur_mt_pos[id][FT_YPOS] = g_xy_data.y4;
                cur_mt_z[id] = FT_MAXZ;
            }

            cur_mt_tch[FT_MT_TCH4_IDX] = id;
            cur_trk[id] = FT_TCH;
            /* do not break */
        }
		case 3:
        {
            ...
            if (rev_y)
            {
                g_xy_data.y3 = INVERT_Y(g_xy_data.y3, ts->platform_data->maxy-1);
            }
            ...
            if (ts->platform_data->use_trk_id)
            {
                cur_mt_pos[FT_MT_TCH3_IDX][FT_XPOS] = g_xy_data.x3;
                cur_mt_pos[FT_MT_TCH3_IDX][FT_YPOS] = g_xy_data.y3;
            }
            else
            {
                cur_mt_pos[id][FT_XPOS] = g_xy_data.x3;
                cur_mt_pos[id][FT_YPOS] = g_xy_data.y3;
                cur_mt_z[id] = FT_MAXZ;
            }

            cur_mt_tch[FT_MT_TCH3_IDX] = id;
            cur_trk[id] = FT_TCH;
            /* do not break */
        }
        case 2:
...
exit_xy_worker:
    if (ts->client->irq != 0 && !ts->insuspend)
    {
...
static int ft5x06_initialize(struct i2c_client *client, struct ft5x06 *ts)
{
    ...
    if (ts->client->irq)
    {
        ...
        retval = request_irq(ts->client->irq, ft5x06_irq, (IRQF_TRIGGER_FALLING | IRQF_NO_SUSPEND), input_device->name, ts);
        if (retval)
        {
...
static int __init enable_pcap_multitouch__func(char *p)
{
        enable_pcap_multitouch = 1;
	printk("ft5x06: Enabling multitouch\r\n");
        return 0;
}
early_param("multitouch", enable_pcap_multitouch__func);
...
static int ft5x06_i2c_ts_probe_dt(struct i2c_client *client, struct ft5x06_platform_data *ts)
{
	...
	ret = of_get_named_gpio_flags(np, "reset-gpio", 0, NULL);
	...
	if(ts->use_mt || enable_pcap_multitouch)
	{
		ts->use_st = 0;
		ts->use_mt = 1;
		ts->use_trk_id = 1;
	...
	of_property_read_u16(np, "xresolution", &ts->maxx);
	of_property_read_u16(np, "yresolution", &ts->maxy);

	ts->wakeup = of_property_read_bool(np, "linux,wakeup");
...
static int ft5x06_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(ts->platform_data == NULL)
	{
		...
		ts->platform_data->maxx = 800;
		ts->platform_data->maxy = 480;
	}

	printk("focaltech irq: %d\r\n", ts->client->irq);
...
static SIMPLE_DEV_PM_OPS(ft5x06_pm, ft5x06_suspend, ft5x06_resume);
