...
static SIMPLE_DEV_PM_OPS(ilitek_pm, ilitek_i2c_suspend, ilitek_i2c_resume);
...
static int ilitek_touch_down(int id, int x, int y, int pressure) {
	...
	input_event(i2c.input_dev, EV_ABS, ABS_MT_TOUCH_MAJOR, 0xe);
	input_event(i2c.input_dev, EV_ABS, ABS_MT_WIDTH_MAJOR, 10);
...
int ilitek_i2c_suspend(struct device *dev)
{
...
/*
   ...
   return status
 */
int ilitek_i2c_resume(struct device *dev)
{
        struct i2c_client *client = to_i2c_client(dev);
	DBG("resume Enter\n");
#ifdef GESTURE
